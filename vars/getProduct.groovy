    ...
    if (options["isPreBuilt"]) {
        ...
        if (options[shaKey] && fileExists("${CIS_TOOLS}/../PluginsBinaries/${options[shaKey]}.${extension}")) {
        	...
        	if (isUnix) {
                sh """
                    ...
                    mv product*.${extension} "${CIS_TOOLS}/../PluginsBinaries/${options[shaKey]}.${extension}"
                """
    		} else {
                bat """
                    IF NOT EXIST "${CIS_TOOLS}\\..\\PluginsBinaries" mkdir "${CIS_TOOLS}\\..\\PluginsBinaries"
                    move product*.${extension} "${CIS_TOOLS}\\..\\PluginsBinaries\\${options[shaKey]}.${extension}"
                """
    		}
        }
        ...
        if (fileExists("${CIS_TOOLS}/../PluginsBinaries/${options.commitSHA}_${osName}.${extension}")) {
        	...
        	if (isUnix) {
                sh """
                    mkdir -p "${CIS_TOOLS}/../PluginsBinaries"
                    mv product*.${extension} "${CIS_TOOLS}/../PluginsBinaries/${options.commitSHA}_${osName}.${extension}"
                """
    		} else {
	            bat """
	                IF NOT EXIST "${CIS_TOOLS}\\..\\PluginsBinaries" mkdir "${CIS_TOOLS}\\..\\PluginsBinaries"
	                move product*.${extension} "${CIS_TOOLS}\\..\\PluginsBinaries\\${options.commitSHA}_${osName}.${extension}"
	            """
    		}
        }
    }
